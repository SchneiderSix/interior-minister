""" @bruin
name: transformation.transform_violencia_domestica
type: python
tags:
  - transformation
depends:
  - ingestion.ingest_tabular
description: Transform violencia_domestica dataset to normalized Parquet
@bruin """
...
RAW_DIR = Path(__file__).resolve().parents[3] / "data" / "raw" / "tabular" / "violencia_domestica"
OUT_PATH = Path(__file__).resolve().parents[3] / "data" / "processed" / "tabular" / "violencia_domestica.parquet"
...
def main() -> None:
    ...
    for file_path in raw_files:
        ...
        year = extract_year_from_filename(file_path.stem)
        if year is not None and "year" not in df.columns:
            df = df.with_columns(pl.lit(year).alias("year"))

        # Cast all columns to Utf8 for safe union across heterogeneous schemas
